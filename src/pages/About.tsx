...
import { Briefcase, GraduationCap, Code, Users, FileText, GitBranch, Rocket, Award, BookOpen, Code2 } from "lucide-react";
...
const About = () => {
  ...
  const entries = t.timeline.entries as unknown as TimelineEntry[];
      ...
      <div className="container max-w-4xl">
        ...
        <motion.div {...fadeUp(0.4)}>
          ...
          <div className="mb-16 text-center">
            <span className="tag-chip text-xs mb-4 inline-flex">{t.timeline.chip}</span>
            <h1 className="font-heading text-4xl sm:text-5xl font-bold tracking-tight mt-3 mb-4">
              {t.timeline.title} <span className="neon-text">{t.timeline.titleAccent}</span>
            </h1>
            <p className="text-muted-foreground max-w-lg mx-auto">{t.timeline.subtitle}</p>

            {/* Legend */}
            <div className="flex items-center justify-center gap-6 mt-8">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full" style={{ background: "hsl(var(--primary))" }} />
                <span className="text-xs font-mono text-muted-foreground">{t.timeline.professional}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full" style={{ background: "hsl(var(--accent))" }} />
                <span className="text-xs font-mono text-muted-foreground">{t.timeline.academic}</span>
              </div>
            </div>
          </div>
                    ...
                    <div className={`flex-1 pl-14 sm:pl-0 ${isLeft ? "sm:pr-12" : "sm:pl-12"}`}>
                      <div className="glass-card p-6 group hover:scale-[1.02] transition-transform duration-300">
                        ...
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          >
                            {entry.type === "professional" ? t.timeline.professional : t.timeline.academic}
                          </span>
                        </div>
